...
export default function DashboardPage() {
  ...
  const { data: txData } = useTransactions(selectedWallet?.id || '', { pageSize: 5 });
  ...
  const quickActions = [
    { href: '/send',    label: 'Send',    icon: '↑', color: 'from-orange-600 to-orange-700' },
    { href: '/receive', label: 'Receive', icon: '↓', color: 'from-emerald-500 to-teal-600' },
    { href: '/history', label: 'History', icon: '⧗', color: 'from-orange-500/60 to-orange-700/60' },
  ];
    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-xl font-bold text-white">{user?.firstName || 'Welcome'} 👋</h1>
        </div>
        <Link href="/profile">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center text-sm font-bold text-white">
      ...
      {/* Balance Card */}
      <div className="relative rounded-3xl overflow-hidden p-6 bg-gradient-to-br from-orange-600/20 to-orange-800/15 border border-orange-500/15 shadow-glow">
        {/* Glow */}
        <div className="absolute inset-0 bg-gradient-to-br from-orange-500/5 to-transparent backdrop-blur-sm" />
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
            {wallets.map((w: any, i: number) => (
              <button
                key={w.id}
                onClick={() => setSelectedIdx(i)}
                className={`flex-shrink-0 glass px-4 py-3 rounded-2xl text-left transition-all ${i === selectedIdx ? 'border-orange-500/40 bg-orange-500/8' : ''}`}
              >
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/history" className="text-xs text-orange-400 hover:text-orange-300">See all</Link>
        </div>
        ...
        {!selectedWallet ? (
          ...
          <div className="text-center py-8">
            ...
            <Link href="/send" className="text-orange-400 text-sm mt-1 inline-block hover:text-orange-300">Make your first transfer →</Link>
          </div>
        ) : (
          <div>
